 ...
 * Gesellschaft
 *
 * @brief Bolt RW biped low level drivers.
 */
...
namespace bolt
{
#define BOLT_RW_NB_MOTOR_BOARD 4
#define BOLT_RW_NB_MOTOR 7
...
public:
    ...
    void send_target_joint_torque(
        const Eigen::Ref<const Eigen::Vector7d> target_joint_torque);
     ...
     */
    const Eigen::Ref<const Eigen::Vector7d> get_joint_positions()
    {
     ...
     * prior to any getter to have up to date data.
     */
    const Eigen::Ref<const Eigen::Vector7d> get_joint_velocities()
    {
     ...
     * prior to any getter to have up to date data.
     */
    const Eigen::Ref<const Eigen::Vector7d> get_joint_torques()
    {
     ...
     */
    const Eigen::Ref<const Eigen::Vector7d> get_joint_target_torques()
    {
     ...
     */
    const Eigen::Ref<const Eigen::Matrix<bool, BOLT_RW_NB_MOTOR, 1> >
    get_motor_enabled()
    {
     ...
     * cards).
     */
    const Eigen::Ref<const Eigen::Matrix<bool, BOLT_RW_NB_MOTOR_BOARD, 1> >
     ...
     * cards).
     */
    const Eigen::Ref<const Eigen::Matrix<int, BOLT_RW_NB_MOTOR_BOARD, 1> >
...
private:
    ...
    Eigen::Matrix<bool, BOLT_RW_NB_MOTOR, 1> motor_enabled_;

    /** @brief Motor readiness to receive commands (ready/not ready). */
    Eigen::Matrix<bool, BOLT_RW_NB_MOTOR, 1> motor_ready_;

    /** @brief Motor Board status (enabled/disabled). */
    Eigen::Matrix<bool, BOLT_RW_NB_MOTOR_BOARD, 1> motor_board_enabled_;

    /** @brief Motor Board Error code (int).
     */
    Eigen::Matrix<int, BOLT_RW_NB_MOTOR_BOARD, 1> motor_board_errors_;
    ...
    Eigen::Vector7d joint_positions_;

    /** @brief Joint velocities. */
    Eigen::Vector7d joint_velocities_;

    /** @brief Joint torques. */
    Eigen::Vector7d joint_torques_;

    /** @brief Target joint torques, reference from the controller. */
    Eigen::Vector7d joint_target_torques_;
