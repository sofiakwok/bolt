...
bool DGMBoltRW::is_in_safety_mode()
{
    ...
    if (bolt_.has_error()) {
      ...
      if (counter % 2000 == 0) {
        printf("DGMBoltRW: Going into safe mode as motor card reports error.\n");
      }
      counter += 1;
    }
    ...
    if (was_in_safety_mode_ || DynamicGraphManager::is_in_safety_mode())
    {
      ...
      if (counter % 2000 == 0)
      {
        printf("DGMBoltRW: is_in_safety_mode.\n");
      }
      counter++;
    }
...
void DGMBoltRW::initialize_hardware_communication_process()
{
    /**
    ...
    ros_user_commands_.push_back(
        ros_node_handle->create_service<mim_msgs::srv::JointCalibration>(
            ...
            std::bind(&DGMBoltRW::calibrate_joint_position_callback,
                      this,
                      std::placeholders::_1,
                      std::placeholders::_2)));
...
void DGMBoltRW::get_sensors_to_map(dynamic_graph_manager::VectorDGMap& map)
{
    ...
    Eigen::Ref<const Eigen::Matrix<bool, BOLT_RW_NB_MOTOR, 1> > motor_enabled =
        bolt_.get_motor_enabled();
    Eigen::Ref<const Eigen::Matrix<bool, BOLT_RW_NB_MOTOR, 1> > motor_ready =
        bolt_.get_motor_ready();
    Eigen::Ref<const Eigen::Matrix<bool, BOLT_RW_NB_MOTOR_BOARD, 1> >
        motor_board_enabled = bolt_.get_motor_board_enabled();
    Eigen::Ref<const Eigen::Matrix<int, BOLT_RW_NB_MOTOR_BOARD, 1> >
        motor_board_errors = bolt_.get_motor_board_errors();

    for (unsigned i = 0; i < BOLT_RW_NB_MOTOR; ++i)
    {
        map_motor_enabled[i] = motor_enabled[i];
        map_motor_ready[i] = motor_ready[i];
    }
    for (unsigned i = 0; i < BOLT_RW_NB_MOTOR_BOARD; ++i)
    {
...
void DGMBoltRW::set_motor_controls_from_map(
    const dynamic_graph_manager::VectorDGMap& map)
{
    ...
    {
        rt_printf(
            "DGMBoltRW::set_motor_controls_from_map: "
            "Error sending controls, %s\n",
            e.what());
    }
}

void DGMBoltRW::calibrate_joint_position_callback(
    mim_msgs::srv::JointCalibration::Request::SharedPtr,
    mim_msgs::srv::JointCalibration::Response::SharedPtr res)
{
    printf("DGMBoltRW::calibrate_joint_position_callback called\n");
    // parse and register the command for further call.
    add_user_command(std::bind(&DGMBoltRW::calibrate_joint_position, this));

    // return whatever the user want
    res->sanity_check = true;
}

void DGMBoltRW::calibrate_joint_position()
{
