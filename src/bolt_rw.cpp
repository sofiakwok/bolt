 ...
 * Gesellschaft
 *
 * @brief Implement the BoltRW class.
 */
...
void BoltRW::initialize(const std::string& network_id)
{
    std::cout << "initializing BoltRW" << std::endl;
    // Network info.
    network_id_ = network_id;

    std::cout << "yaml path: " << ODRI_CONTROL_INTERFACE_RW_YAML_PATH << std::endl;
    // Main driver interface.
    robot_ = odri_control_interface::RobotFromYamlFile(
        network_id_, ODRI_CONTROL_INTERFACE_RW_YAML_PATH);

    calib_ctrl_ = odri_control_interface::JointCalibratorFromYamlFile(
        ODRI_CONTROL_INTERFACE_RW_YAML_PATH, robot_->joints);

    // Use a serial port to read slider values.
    // serial_reader_ = std::make_shared<slider_box::SerialReader>(
    //     "auto", BoltRW_NB_SLIDER + 1);

    // Initialize the robot.
    robot_->Init();
}

void BoltRW::wait_until_ready()
{
    robot_->WaitUntilReady();
}

void BoltRW::acquire_sensors()
{
    // Acquire the data.
...
void BoltRW::send_target_joint_torque(
...
void BoltRW::request_calibration(
    const Eigen::Ref<const Eigen::VectorXd> home_offset_rad)
{
    printf("BoltRW::calibrate called\n");
